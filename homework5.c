...
// 404 error and HTML message when a request asks for a file that does not exist.
char * error = "<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 3.2 Final//EN\"><html>"
               "<title>Error: 404</title>"
               "<body>"
               "<marquee behavior='alternate'><marquee width='250' height='250'><font color='red' size='16'><b>ERROR 404!</b></font></marquee></marquee>"
               "</body>";

char * request_str = "HTTP/1.0 200 OK\r\n"
"Content-type: text/html; charset=UTF-8\r\n\r\n";
// type of received file: image/icon        
char * request_strICO = "HTTP/1.0 200 OK\r\n"
        "Content-type: image/icon; charset=UTF-8\r\n\r\n";
// type of received file: image/gif        
char * request_strGIF = "HTTP/1.0 200 OK\r\n"
        "Content-type: image/gif; charset=UTF-8\r\n\r\n";
// type of received file: image/jpg        
char * request_strJPG = "HTTP/1.0 200 OK\r\n"
        "Content-type: image/jpg; charset=UTF-8\r\n\r\n";
// type of received file: image/png
char * request_strPNG = "HTTP/1.0 200 OK\r\n"
        "Content-type: image/png; charset=UTF-8\r\n\r\n";
// type of received file: pdf
...
void serve_request(int client_fd) {
    ...
    // strstr checks for str file extension inside requested file string
    if(strstr(requested_file, ".pdf"))
    {
        send(client_fd,request_strPDF, strlen(request_strPDF),0);
    ...
    if(strstr(requested_file, ".ico"))
    {
        send(client_fd,request_strICO, strlen(request_strICO),0);

    }
    if(strstr(requested_file, ".gif"))
    {
        send(client_fd,request_strGIF, strlen(request_strGIF),0);

    }
    if(strstr(requested_file, ".jpg"))
    {
        send(client_fd,request_strJPG, strlen(request_strJPG),0);

    }
    if(strstr(requested_file, ".png"))
    {
        send(client_fd,request_strPNG, strlen(request_strPNG),0);
    ...
    // not able to open file and send error message
    if(read_fd == -1) 
    {
    ...
    if(d)
    {
	// append filename to index.html
        strcat(filename, "/index.html");
        check = stat(filename, &index);
        
	// check if file exists; then webserver read it bytes by bytes and send to client
	if(check >= 0)
        {
            send(client_fd,request_str, strlen(request_str),0);
            read_fd = open(filename,0,0);
	...
	// else if file is not in the directory
        else 
	{
           send(client_fd, request_str, strlen(request_str), 0);
           sprintf(buffer, index_hdr, filename, filename);
		...
		{  
		    sprintf(buffer, index_body, dir->d_name, dir->d_name);
                    send(client_fd, buffer, strlen(buffer),0);
                    //sprintf(buffer, index_body, dir->d_name);
                    printf("%s\n", dir->d_name);    
            	}

            }
	    // done with reading the directory, close and send	
    ...
    close(read_fd);//close the reading.
    close(client_fd);//close the client_side sending.
    return;
}
